...
NOTE: Die Komponenten einer IsyFact-Anwendung werden in <<IsyFactReferenzarchitektur>> beschrieben.
...
Ansonsten werden die Vorgaben für das Management-Interface von Batches im Konzept <<BatchDetailkonzept>> beschrieben.
...
Für weitere Details siehe <<DatenzugriffDetailkonzept>>.
...
Nähere Details zu dem Platzhalter-Mechanismus sind in <<AnwendungskernDetailkonzept>> nachzulesen.

[[log4j-konfiguration]]
=== Log4j-Konfiguration

Die eigentliche Log4j-Konfiguration wird in <<LoggingKonzept>> beschrieben.
...
Details dazu sind im Konzept <<LoggingKonzept>> nachzulesen.
