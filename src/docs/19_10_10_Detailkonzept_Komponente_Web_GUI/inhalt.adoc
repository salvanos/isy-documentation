...
welche die für den Anwendungsfall notwendigen Anforderungen umsetzt <<IsyFactReferenzarchitektur>>.
...
Für die Namenskonvention zu Java-Klassen und Paketen wird hier auf das Dokument <<Programmierkonventionen>> verwiesen.
...
Die eigentliche Validierung der Eingabedaten sollte im Anwendungskern erfolgen (z.B. durch Nutzung
der plis-validation).
...
Hierzu finden sich weitere Informationen im Dokument <<FehlerbehandlungKonzept>>.
...
Die verwendeten Rechte müssen durch die Rollenrechte-Konfiguration (siehe <<SicherheitNutzerdok>>)
...
Im letzten Abschnitt wird aufgezeigt, welche Konfigurationen durchgeführt werden müssen, sofern die PLIS-Web vollständig verwendet wird.
...
Für die Persistierung von Spring Beans kommt JPA mit Hibernate zum Einsatz (siehe <<Spring>>, <<JPA>> und <<DatenzugriffDetailkonzept>>).
...
Die notwendigen Konfigurationen und weitere Details sind in <<LoggingKonzept>> beschrieben.
...
[[konfiguration-einer-anwendung-unter-nutzung-der-plis-web]]
== Konfiguration einer Anwendung unter Nutzung der PLIS-Web
...
.  Für Anwendungen, die Breadcrumbs unterstützen, kann die Bean `BreadcrumbAwareDbBasedFlowExecutionRepository`
 mit der neuen plis-web-Klasse `BreadcrumbAwareFlowExecution­Repository` ersetzt werden.
